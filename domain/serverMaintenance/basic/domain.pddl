...
(:action DISABLE-SERVER
  :parameters
   (?server - server
    ?backend - backend)
  :precondition
   (and (in ?backend ?server) (enabled ?server) ( > (active ?backend) 1) (not (maintanance-mode)) )
  :effect
...
(:action START-MAINTANANCE
	...
	:effect (and (maintanance-mode))
)

(:action END-MAINTANANCE
	:parameters ()
	:precondition (and (maintanance-mode))
	:effect (and (not (maintanance-mode)))
)
